...
\begin{enumerate}[leftmargin=*]
  ...
  \item A two-layer temporal mask stabilization scheme: a camera-space EMA smoother with a conservative-blend policy that suppresses stochastic segmentation variance, and a BEV-space forward motion compensator that applies a geometrically exact uniform pixel shift to successive BEV masks before temporal averaging, exploiting the uniform-translation property of planar homographic projection under forward ego-motion.
...
This chapter extends the offline perception pipeline described in Chapter~\ref{ch:methodology} to a real-time closed-loop system capable of autonomous sidewalk navigation. We describe eight additions: (1)~a two-layer temporal mask stabilization scheme, (2)~a lightweight object-detection module for dynamic obstacle awareness, (3)~GPS-based waypoint navigation for global route following, (4)~occupancy-aware path planner robustness for handling BEV collapse and stale-path lock, (5)~precise steering-angle and speed computation, (6)~a serial command protocol for the scooter platform, (7)~temporal smoothing for heading and speed stability, and (8)~a live demonstration system that integrates all components.
...
% -----------------------------------------------------------------
\section{Temporal Mask Stabilization}
\label{sec:temporal_stabilization}

Raw per-frame segmentation masks contain two distinct sources of instability that must be addressed before BEV projection and path extraction can be reliable. The first is \emph{model variance}: stochastic fluctuation in the predicted boundary between frames even when the scene is static, caused by minor changes in illumination, motion blur, and the approximate nature of learned segmentation. The second is \emph{ego-motion shift}: the apparent position of the sidewalk region changes deterministically as the scooter advances, causing the BEV mask to translate forward with each frame. We address both with a two-layer stabilization strategy applied at different stages of the perception pipeline.

% -----------------------------------------------------------------
\subsection{Camera-Space Exponential Moving Average}
\label{sec:cam_ema}

The first layer suppresses model variance by applying an exponential moving average (EMA) to consecutive binary segmentation masks in the image frame. The running estimate is updated each frame as:
\begin{equation}
\hat{M}_t^{\text{cam}} = \alpha_{\text{eff}} \cdot M_t + (1 - \alpha_{\text{eff}}) \cdot \hat{M}_{t-1}^{\text{cam}}
\label{eq:cam_ema}
\end{equation}
where $M_t \in \{0,1\}^{H \times W}$ is the raw binary sidewalk mask at frame $t$ and $\alpha_{\text{eff}} \in (0,1)$ is the effective current-frame weight. Rather than using a fixed $\alpha$, we employ a conservative-blend policy that reduces $\alpha_{\text{eff}}$ when the consecutive-frame intersection over union (IoU) falls below a consistency threshold $c_{\text{thresh}}$, since a large IoU drop signals a probable camera shake or sudden scene change that should not be incorporated at full weight:
\begin{equation}
\alpha_{\text{eff}} =
\begin{cases}
\alpha / 4, & \text{IoU}(\hat{M}_{t-1}^{\text{cam}},\, M_t) < c_{\text{thresh}} \\
\alpha / 2, & \text{IoU}(\hat{M}_{t-1}^{\text{cam}},\, M_t) < 0.5 \\
\alpha,     & \text{otherwise}
\end{cases}
\label{eq:cam_conservative_blend}
\end{equation}

On the first call in a session, the smoother bypasses Equation~\ref{eq:cam_ema} and sets $\hat{M}_0^{\text{cam}} = M_0$ directly. This fast-path initialization ensures that an obstacle appearing for the first time is immediately visible at full strength, rather than being attenuated by the EMA history from a previous (obstacle-free) estimate. Parameters $\alpha = 0.65$ and $c_{\text{thresh}} = 0.20$ were selected via the 35-combination grid sweep described in Section~\ref{sec:smoother_sweep}; the resulting smoothed stability is 99.6\% of consecutive frame pairs with IoU $\geq 0.85$.

% -----------------------------------------------------------------
\subsection{BEV-Space Forward Motion Compensation}
\label{sec:bev_motion_comp}

The camera-space EMA described above does not account for the deterministic shift in the BEV mask caused by forward ego-motion. Applying temporal averaging in the image frame before warping to BEV is geometrically incorrect for a moving platform: under forward translation, the apparent position of a sidewalk pixel moves upward in the image at a rate that depends on its depth, making per-pixel motion non-uniform and scene-dependent. Correcting for this in image space would require per-pixel depth estimates---unavailable on a monocular system.

BEV space eliminates this difficulty. A key geometric consequence of a fixed planar homography under the flat-ground assumption is that forward ego-motion corresponds to a \emph{uniform downward translation} of the entire BEV mask~\cite{hartley_zisserman}. Specifically, if the scooter advances by $\delta$ meters in $\Delta t$ seconds, every point in the BEV mask shifts by the same amount:
\begin{equation}
\Delta y_{\text{bev}} = v \cdot \Delta t \cdot \frac{h_{\text{bev}}}{d_{\text{fwd}}}
\label{eq:bev_shift}
\end{equation}
where $v$ is the forward speed in m/s, $h_{\text{bev}} = 500$~px is the BEV image height (the forward-range axis), and $d_{\text{fwd}} = 10$~m is the maximum forward look-ahead distance, yielding a scale of $50$~px/m. At a representative speed of $v = 1.0$~m/s and frame rate $f = 9$~Hz, the per-frame shift is $\Delta y_{\text{bev}} \approx 5.6$~px---modest but sufficient to misalign successive masks if uncompensated.

This uniformity is what makes BEV the correct domain for ego-motion compensation: it decouples forward translation from depth, so the shift can be computed analytically without any learned module. State-of-the-art multi-camera perception systems such as BEVFormer~\cite{bevformer} address the same problem using deformable spatial attention that aligns BEV feature volumes across frames using full 6-DoF pose transforms. Our approach specializes this to the 1-DoF forward-motion case afforded by a single fixed camera on a nominally planar scooter, yielding a deterministic closed-form solution at negligible computational cost. The geometric validity of the approximation rests on the Lift, Splat, Shoot framework~\cite{lift_splat_shoot}, which establishes that a calibrated planar homography produces a metrically consistent BEV grid in which rigid body motion of the camera corresponds to a shift of the grid coordinates.

In practice, the shift at frame $t$ is estimated from the speed available at the end of the previous control cycle:
\begin{equation}
\Delta y_t = \hat{v}_{t-1} \cdot \frac{1}{f_{\text{fps}}} \cdot \frac{h_{\text{bev}}}{d_{\text{fwd}}}
\label{eq:bev_shift_impl}
\end{equation}
where $\hat{v}_{t-1}$ is the speed command issued at frame $t-1$, available without causal violation. The previous BEV mask estimate is translated upward by $\Delta y_t$ pixels before the EMA update, aligning it to the expected viewpoint of frame $t$:
\begin{equation}
\hat{M}_t^{\text{bev}} = \beta \cdot M_t^{\text{bev}} + (1-\beta) \cdot T_{-\Delta y_t}\!\left(\hat{M}_{t-1}^{\text{bev}}\right)
\label{eq:bev_ema}
\end{equation}
where $M_t^{\text{bev}} = \mathcal{H}(M_t^{\text{cam}})$ is the freshly warped BEV mask, $T_{-\Delta y}$ denotes an upward affine translation by $\Delta y$ pixels, and $\beta = 0.55$ is the BEV-layer EMA weight (tuned separately from $\alpha$ in the camera layer). The translated prior $T_{-\Delta y_t}(\hat{M}_{t-1}^{\text{bev}})$ represents where the previous sidewalk estimate is expected to appear in the current BEV frame after the scooter has advanced; blending it with $M_t^{\text{bev}}$ suppresses temporal noise while keeping the mask geometrically aligned with the current position.
...
% -----------------------------------------------------------------
\section{Occupancy-Aware Path Planner Robustness}
\label{sec:path_robustness}

The BEV path extractor operates on a segmentation mask that can undergo transient \emph{occupancy collapse}---frames in which the projected BEV sidewalk region shrinks dramatically due to segmentation under-confidence, glare, or textural ambiguity near the edges of the camera's view. Without mitigation, such collapses cause the predictor to hold a stale path estimate and the controller to apply a persistent lateral bias that outlasts the collapse window. This section describes the occupancy-aware mechanisms introduced to address this failure mode.

\subsection{BEV Occupancy Gating in the Frame Predictor}

The predictive frame-reuse module skips full BEV segmentation inference on frames where the predicted BEV occupancy is expected to resemble the previous frame, reducing latency at the cost of a small accuracy trade-off. A failure mode arises when the predictor continues to skip while actual BEV occupancy has dropped sharply, propagating a stale high-confidence path derived from a mask that no longer reflects the current scene.

We address this with two additions:
\begin{enumerate}
  \item \textbf{Occupancy tracking:} After each compute frame, the BEV occupancy ratio $r_{\mathrm{occ}} \in [0,1]$ (fraction of BEV pixels classified as sidewalk) is stored in the predictor state.
  \item \textbf{Skip blocking:} Before allowing a skip, the predictor checks whether the stored $r_{\mathrm{occ}}$ falls below a minimum evidence threshold. If so, the skip is blocked and full inference is forced. Additionally, if the predicted BEV mask is sparse, the predicted path is invalidated and the stale path state is cleared to prevent prolonged steering lock.
\end{enumerate}

\subsection{Low-Evidence Mode and Aggressive Hold Decay}

Even when the predictor runs full inference every frame, transient BEV collapses can still cause the path extractor's hold mechanism to accumulate lateral bias over multiple frames. We introduce a \emph{low-evidence mode}: when $r_{\mathrm{occ}}$ falls below a configurable floor threshold, the extractor activates an aggressive hold-decay policy that progressively attenuates the lateral component of the retained path, steering the output toward the image center as evidence degrades. When $r_{\mathrm{occ}}$ recovers, the extractor exits low-evidence mode and resumes normal candidate scoring.

\subsection{Center- and Continuity-Aware Candidate Scoring}

The path extractor evaluates multiple candidate paths extracted from the skeleton graph and selects the highest-scoring one. The original scoring function used only path length and forward progress. We add two terms:
\begin{itemize}
  \item \textbf{Center preference:} A penalty proportional to the lateral offset of the path's near-ego segment from the BEV centerline. This biases selection toward paths that depart the ego zone along the image center axis, suppressing spurious short branches that point toward the BEV image edges.
  \item \textbf{Temporal continuity:} A bonus for candidate paths whose near-ego heading agrees with the previously accepted heading, reducing abrupt direction changes at junction frames.
\end{itemize}

\subsection{Skeleton Geodesic Fallback Path}

When the skeleton graph is fragmented---a common consequence of aggressive branch pruning on short BEV masks---the Dijkstra-based extractor may produce only zero or one viable graph candidate. In such frames, the system falls back to a \emph{skeleton geodesic path}: a direct shortest-path computation over the raw skeleton pixel graph from the ego-nearest skeleton seed to the most forward-progressing far endpoint, using a pixel-level Dijkstra traversal. The resulting pixel-space polyline is converted to metric coordinates and ego-anchored.

The skeleton fallback is preferred over the mask-centerline fallback, which uses only the column-wise centroid of the BEV mask and carries no skeleton geometry. An override rule additionally forces skeleton candidate selection when the top-scoring graph candidate is both short in forward progress ($<$\SI{0.8}{m}) and laterally biased near the ego point---a signature of near-ego triangle branch artifacts that cause false early turns.

\subsection{Bounded Discontinuity Hold in Pure Pursuit}

When the selected path changes abruptly between frames---for example, when the planner transitions from a held stale path to a freshly recomputed one---the instantaneous heading change can be large enough to cause physical instability. We add a \emph{bounded discontinuity hold}: when the heading implied by the new path differs from the current heading by more than a threshold, the controller holds the previous heading for up to $N_{\mathrm{hold}}$ frames while gradually blending toward the new path. If no acceptable path is found within $N_{\mathrm{hold}}$ frames, the hold is released and the system falls back to a centerline estimate, preventing indefinite stale-path lock while still absorbing brief acquisition transients.

Path source (which component produced the accepted path) and BEV occupancy ratio are logged to CSV and overlaid on the BEV HUD at runtime, enabling post-hoc diagnosis of collapse windows and fallback activations.
...
% -----------------------------------------------------------------
\subsection{Segmentation Checkpoint Benchmark}
\label{sec:seg_checkpoint_benchmark}

To select the best-performing SegFormer checkpoint for deployment, we evaluated 11 candidate model checkpoints on a representative sidewalk video using a temporal stability metric. Stability is defined as the fraction of consecutive frame pairs whose raw (unsmoothed) segmentation IoU exceeds 0.85---the minimum threshold for visually flicker-free output visible to a demo observer. Each checkpoint was evaluated on 300 frames sampled from \texttt{test\_video\_mar3\_1\_h264.mp4} on a workstation equipped with an NVIDIA Quadro P620 GPU, Intel Core i7 CPU, and 32~GB RAM.

Frame classification follows three categories: \emph{stable} (IoU $\geq 0.85$), \emph{failure} (IoU = 0.0 with a non-empty previous mask, indicating a total segmentation miss), and \emph{unstable} (all other cases). Table~\ref{tab:checkpoint_benchmark} reports the results sorted by percent stable frames.

\begin{table}[h]
\centering
\caption{Temporal stability benchmark across 11 SegFormer checkpoints on 300 consecutive frames. Stable = consecutive-frame IoU $\geq$ 0.85. Failure = IoU = 0.0 with non-empty previous mask. \texttt{checkpoint-5000} was excluded due to missing model weight files.}
...
\texttt{checkpoint-3000}        & 75.6          & 0.0 & 0.888 & 0.933 \\
\texttt{checkpoint-4500}        & 73.6          & 0.0 & 0.881 & 0.936 \\
\texttt{checkpoint-2500}        & 70.9          & 0.0 & 0.887 & 0.930 \\
\texttt{checkpoint-3500}        & 69.6          & 0.0 & 0.871 & 0.936 \\
\texttt{checkpoint-500}         & 67.6          & 1.7 & 0.818 & 0.901 \\
\bottomrule
\end{tabular}
\end{table}

The \texttt{my-segformer-road} checkpoint achieves 99.3\% stable frames and a mean consecutive-frame IoU of 0.968, outperforming all other checkpoints by at least 5.0 percentage points. The second-best checkpoint (\texttt{checkpoint-4000} at 94.3\%) uses the same architecture but corresponds to an earlier training epoch with less data exposure. Notably, the \texttt{my-segformer-road\_new} checkpoint---the previous deployment model before this benchmark---ranks only fourth at 88.0\%, below the 90\% stability target. The \texttt{checkpoint-500} checkpoint is the only model that produces genuine segmentation failures (1.7\% of frames), consistent with an undertrained model with an unstable road/non-road decision boundary.

Based on these results, \texttt{my-segformer-road} was selected as the deployment model. Its 99.3\% raw stability already exceeds the 90\% target by 9.3 percentage points without any temporal smoothing, providing a strong baseline for the subsequent EMA tuning stage.

% -----------------------------------------------------------------
\subsection{Temporal Smoothing Parameter Sweep}
\label{sec:smoother_sweep}

With the best checkpoint established, we tuned the EMA temporal smoother to push stability from the 99.3\% raw baseline toward the theoretical ceiling. The smoother maintains a running weighted average of consecutive binary sidewalk masks:
\begin{equation}
\hat{M}_t = \alpha \cdot M_t + (1 - \alpha) \cdot \hat{M}_{t-1}
\label{eq:ema_mask}
\end{equation}
where $\alpha \in (0,1)$ is the current-frame EMA weight and a conservative-blend branch reduces the effective $\alpha$ when the consecutive-frame IoU falls below a consistency threshold $c_{\text{thresh}}$ (indicating a probable shake artifact or severe appearance change).

We performed a grid search over 7 values of $\alpha \in \{0.25, 0.30, 0.35, 0.40, 0.45, 0.55, 0.65\}$ and 5 values of $c_{\text{thresh}} \in \{0.20, 0.25, 0.30, 0.40, 0.50\}$, yielding 35 combinations in total. To avoid redundant GPU inference, raw segmentation masks were collected in a single forward pass; all 35 parameter combinations were then replayed on CPU. The sweep was evaluated on 500 frames---a larger window than the checkpoint benchmark to improve statistical confidence.

Table~\ref{tab:smoother_sweep} reports results collapsed over $c_{\text{thresh}}$, which exhibited zero sensitivity to threshold variation across all 35 combinations. This insensitivity is expected: with 99.3\% of frames already stable, the conservative-blend branch is triggered in fewer than 0.7\% of frame transitions, making $\alpha$ magnitude the sole determinant of smoothing gain.

\begin{table}[h]
\centering
\caption{EMA temporal smoother stability as a function of $\alpha$, evaluated on 500 frames of \texttt{test\_video\_mar3\_1\_h264.mp4}. Results are identical across all five $c_{\text{thresh}}$ values tested; entries represent averages over those values. Raw baseline (unsmoothed): 99.3\%.}
\label{tab:smoother_sweep}
\begin{tabular}{rcc}
\toprule
$\alpha$ & Stable (\%) & Failure (\%) \\
\midrule
0.65 & \textbf{99.6} & 0.0 \\
0.55 & 99.6          & 0.0 \\
0.45 & 97.6          & 0.0 \\
...
Both $\alpha = 0.55$ and $\alpha = 0.65$ achieve 99.6\% stability. We select $\alpha = 0.65$ because a higher current-frame weight preserves responsiveness to newly appearing obstacles---satisfying the requirement that dynamic agents become visible in the segmentation output within 2--3 frames of first appearance. The floor at $\alpha = 0.25$ (92.4\% stable) demonstrates that the smoother degrades gracefully: even under aggressive temporal averaging, stability remains above the 90\% target.

The final configuration---\texttt{my-segformer-road} with $\alpha = 0.65$ and $c_{\text{thresh}} = 0.20$---achieves \textbf{99.6\% temporally stable frames} on the 500-frame evaluation window. This represents a 0.3~percentage-point improvement over the raw unsmoothed baseline and a 9.6~point margin above the 90\% stability target. Zero segmentation failures (total misses) were observed at any tested alpha value.
...
\section{Path Planning Robustness Evaluation}
\label{sec:path_robustness_results}

To quantify the benefit of the occupancy-aware robustness improvements described in Section~\ref{sec:path_robustness}, we ran the full pipeline on a held-out campus recording (\texttt{test\_video\_june\_03\_3.mp4}, 2,058 frames, $\approx$3.8~min at 9~FPS) under three configurations: the baseline (predictor enabled, original hold logic), a diagnostic no-predict baseline (predictor disabled entirely), and Experiment~1 (E1: selected occupancy-aware improvements).

\subsection{Heading Error Reduction}

Heading error is measured as the absolute steering angle $|\theta|$ (degrees) output by the vision path pipeline.
Table~\ref{tab:robustness_heading} compares the configurations globally and within a known BEV collapse window (frames 1520--1600) where intermittent occupancy drops were identified during diagnosis.

\begin{table}[h]
\centering
\caption{Path planning robustness evaluation on \texttt{test\_video\_june\_03\_3.mp4} (2,058~frames). Collapse window = frames 1520--1600 (80~frames with confirmed BEV occupancy drop). E1 = selected occupancy-aware improvements (Section~\ref{sec:path_robustness}).}
\label{tab:robustness_heading}
\begin{tabular}{lcccc}
\toprule
Configuration & Mean $|\theta|$ ($^\circ$) & P95 $|\theta|$ ($^\circ$) & Collapse mean ($^\circ$) & Collapse $|\theta|{>}8^\circ$ (frames) \\
\midrule
Baseline (predict ON)     & 0.824 & 2.693 & 5.85 & 38 \\
E1 (predict ON)           & \textbf{0.690} & \textbf{2.407} & \textbf{2.58} & \textbf{2} \\
\midrule
No-predict before E1      & 19.98 & 28.13 & ---  & --- \\
No-predict after E1       & 0.776 & 2.646 & ---  & --- \\
\bottomrule
\end{tabular}
\end{table}

The E1 configuration reduces mean heading error by 16.3\% and P95 heading error by 10.6\% across all frames. The most substantial improvement is in the collapse window: the number of frames exceeding $8^\circ$ drops from 38 to~2 (a 94.7\% reduction), and the collapse-window mean error decreases from $5.85^\circ$ to $2.58^\circ$ (55.9\% reduction). The no-predict diagnostic is particularly revealing: without the predictor, the baseline exhibits severe persistent steering lock (mean $|\theta| = 19.98^\circ$, P95~$= 28.13^\circ$), confirming that the original hold mechanism was accumulating lateral bias independently of the predictor. After the E1 fix, no-predict operation recovers to mean $|\theta| = 0.776^\circ$ and P95~$= 2.646^\circ$---comparable to the predict-ON baseline---confirming that the stale-lock failure originated in the path-hold logic rather than in the predictor itself.

\subsection{Path Source Distribution}

Table~\ref{tab:path_source_dist} shows the distribution of accepted path sources for the primary E1 run.

\begin{table}[h]
\centering
\caption{Path source distribution for E1 (predict ON) run on \texttt{test\_video\_june\_03\_3.mp4} (2,057~accepted frames). Sources reported per-frame in the \texttt{path\_source} CSV field.}
\label{tab:path_source_dist}
\begin{tabular}{lrr}
\toprule
Path Source & Frames & Fraction (\%) \\
\midrule
\texttt{predict}              & 1457 & 70.8 \\
\texttt{graph}                & 523  & 25.4 \\
\texttt{fallback\_centerline} & 55   &  2.7 \\
\texttt{fallback\_hold}       & 22   &  1.1 \\
\bottomrule
\end{tabular}
\end{table}

The predictor path dominates at 70.8\%, as expected given the inter-frame consistency of campus sidewalk geometry at 9~FPS. Dijkstra graph paths account for 25.4\% of frames, covering compute frames where predictor skip is blocked by low occupancy. The two degenerate fallbacks (centerline and hold) together account for fewer than 4\% of frames, confirming that the graph and predictor layers resolve the vast majority of frames without resorting to geometry-free fallbacks.
...
The closed-loop extension demonstrates that the perception-to-control stack can operate within the latency budget for pedestrian-speed navigation. A systematic benchmark across 11 SegFormer checkpoints identified \texttt{my-segformer-road} as the most stable model variant, achieving 99.3\% of consecutive frame pairs with IoU $\geq 0.85$ on raw unsmoothed output. A subsequent 35-combination EMA parameter sweep ($\alpha \times c_{\text{thresh}}$) raised smoothed stability to 99.6\%---a 9.6~percentage-point margin above the 90\% target---with $\alpha = 0.65$ selected to preserve obstacle responsiveness. Occupancy-aware path planner robustness improvements reduced mean absolute heading error from $0.824^\circ$ to $0.690^\circ$ overall, and cut the number of frames exceeding $8^\circ$ during BEV collapse windows from 38 to 2 (a 94.7\% reduction). Temporal smoothing of heading estimates further reduces frame-to-frame jitter, and the live demonstration system validates the approach on real sidewalks with a consumer camera.
...
Future work will extend the system along five directions:

\begin{enumerate}
  \item \textbf{BEV obstacle projection from YOLO detections.} YOLOv8-nano object detection is currently used only for speed modulation via monocular distance estimation (Section~\ref{sec:object_detection}). A natural next step is to project each detected bounding box onto the BEV plane using the same homography, converting it into a 2-D exclusion zone that the skeleton-based path planner must route around. This would enable the path to actively deviate from an oncoming pedestrian rather than only slowing down, and would allow the planner to find an alternative branch when the primary path is fully blocked by a static obstacle.

  \item \textbf{Additional fine-tuning data.} The segmentation model was trained on 300 hand-labeled frames supplemented by 2{,}300 pseudo-labeled frames. Collecting and annotating additional frames from diverse conditions---wet pavement, nighttime, steep ramps, and dense pedestrian environments---using a labeling tool such as LabelStudio and retraining with the teacher--student pipeline is expected to further improve boundary accuracy and reduce false positives at sidewalk edges.

  \item \textbf{BEV recalibration for the final camera mount.} The current homography was calibrated for a hand-held camera in testing conditions. Once the scooter camera mount is finalized, recalibrating the four-point ground-plane correspondence for the fixed pose is a critical step; a well-conditioned homography (condition number $\ll 10^3$, compared to the current $\sim 10^6$) is expected to raise the path-detection rate from the current 2\% to 60--80\% on typical sidewalk footage.

  \item \textbf{Cloud-offloaded inference.} An open question is whether offloading segmentation to a cloud GPU running a larger model (e.g., SegFormer-B4 or Mask2Former) improves navigation quality enough to justify the round-trip network latency. Compressing and streaming frames to a remote endpoint, receiving mask predictions, and feeding them into the local BEV and control stack would enable higher-capacity perception without on-board GPU hardware. Quantifying the latency--quality tradeoff relative to the local CPU-B0 baseline is a natural direction for future study.
